#input: bids(root='results',suffix='b0.nii.gz',desc='topup',method='jac',datatype='dwi',**config['subj_wildcards'])
...
rule import_avg_b0:
    ...
    output:
        bids(root='results',subfolder='bet_from-b0',suffix='b0.nii.gz',desc='topup',datatype='dwi',**config['subj_wildcards']),
    shell:
        'cp {input} {output}'
...
rule n4_avg_b0:
    input:
        bids(root='results',subfolder='bet_from-b0',suffix='b0.nii.gz',desc='topup',datatype='dwi',**config['subj_wildcards']),
    output:
        bids(root='results',subfolder='bet_from-b0',suffix='b0.nii.gz',desc='n4',datatype='dwi',**config['subj_wildcards']),
...
rule rescale_avg_b0:
    input:
        bids(root='results',subfolder='bet_from-b0',suffix='b0.nii.gz',desc='n4',datatype='dwi',**config['subj_wildcards']),
    output:
        bids(root='results',subfolder='bet_from-b0',suffix='b0.nii.gz',desc='rescale',datatype='dwi',**config['subj_wildcards']),
    container: config['singularity']['prepdwi']
    shell:
...
rule bet_avg_b0_default_frac:
    input:
        bids(root='results',subfolder='bet_from-b0',suffix='b0.nii.gz',desc='rescale',datatype='dwi',**config['subj_wildcards']),
    output:
        bids(root='results',subfolder='bet_from-b0',suffix='b0.nii.gz',desc='bet',datatype='dwi',**config['subj_wildcards']),
    container: config['singularity']['prepdwi']
    shell:
        'bet {input} {output}'
...
rule bet_avg_b0_custom_frac:
    ...
    params:
        frac = '0.{frac}'
    output:
        bids(root='results',subfolder='bet_from-b0',suffix='b0.nii.gz',desc='bet',frac='{frac}',datatype='dwi',**config['subj_wildcards'])
    container: config['singularity']['prepdwi']
    shell:
