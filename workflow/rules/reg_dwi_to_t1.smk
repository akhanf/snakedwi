rule import_t1w:
    input:
        t1w = config['input_path']['t1w_preproc'],
    output:
        t1w = bids(root='results',datatype='anat',suffix='T1w.nii.gz',desc='preproc',**config['input_wildcards']['t1w_preproc']),
...
rule reg_aladin_b0_to_t1:
    input: 
        ...
        avgb0 =  bids(root='results',suffix='b0.nii.gz',desc='topup',method='jac',datatype='dwi',**config['subj_wildcards']),
    output: 
        warped_avgb0 = bids(root='results',suffix='avgb0.nii.gz',space='T1w',desc='topup',datatype='dwi',**config['subj_wildcards']),
...
rule create_cropped_ref:
    input:
        warped_avgb0 = bids(root='results',suffix='avgb0.nii.gz',space='T1w',desc='topup',datatype='dwi',**config['subj_wildcards']),
    output:
        cropped_avgb0 = bids(root='results',suffix='avgb0.nii.gz',space='T1w',desc='topup',proc='crop',datatype='dwi',**config['subj_wildcards']),
    container: config['singularity']['prepdwi']
    shell:
...
rule create_cropped_ref_t1_resolution:
    input:
        cropped_avgb0 = bids(root='results',suffix='avgb0.nii.gz',space='T1w',desc='topup',proc='crop',datatype='dwi',**config['subj_wildcards']),
    output:
        avgb0_crop_resample = bids(root='results',suffix='avgb0.nii.gz',space='T1w',desc='topup',proc='crop',res='T1w',datatype='dwi',**config['subj_wildcards']),
    shell:
        'cp {input} {output}'

rule create_cropped_ref_dwi_resolution:
    input:
        cropped = bids(root='results',suffix='avgb0.nii.gz',space='T1w',desc='topup',proc='crop',datatype='dwi',**config['subj_wildcards']),
        res_txt_orig = bids(root='results',suffix='b0.resolution_mm.txt',desc='topup',method='jac',datatype='dwi',**config['subj_wildcards']),
    output:
        resampled = bids(root='results',suffix='avgb0.nii.gz',space='T1w',desc='topup',proc='crop',res='orig',datatype='dwi',**config['subj_wildcards']),
    container: config['singularity']['prepdwi']
    shell:
...
rule create_cropped_ref_custom_resolution:
    ...
    output:
        resampled = bids(root='results',suffix='avgb0.nii.gz',space='T1w',desc='topup',proc='crop',res='custom',datatype='dwi',**config['subj_wildcards']),
    container: config['singularity']['prepdwi']
    shell:
...
rule resample_dwi_to_t1w:
    input:
        ref = bids(root='results',suffix='avgb0.nii.gz',space='T1w',desc='topup',proc='crop',res=config['resample_dwi']['resample_scheme'],datatype='dwi',**config['subj_wildcards']),
