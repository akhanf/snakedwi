...
def get_dwi_ref (wildcards):
    ...
    if num_scans > 1 and not config['no_topup']:
        return bids(root='work',suffix='b0.nii.gz',desc='topup',method='jac',datatype='dwi',**config['subj_wildcards'])
    else:
        return bids(root='work',suffix='b0.nii.gz',desc='degibbs',datatype='dwi',**config['subj_wildcards'])
        

rule cp_dwi_ref:
    input: get_dwi_ref
    output: bids(root='work',suffix='b0.nii.gz',desc='dwiref',datatype='dwi',**config['subj_wildcards']),
    group: 'subj'
    shell: 'cp {input} {output}'    
...
def get_eddy_topup_input (wildcards):
    ...
    if num_scans > 1 and not config['no_topup']:
        topup_inputs = { filename : bids(root='work',suffix=f'{filename}.nii.gz',datatype='dwi',**config['subj_wildcards']).format(**wildcards) for filename in ['topup_fieldcoef','topup_movpar']}
        return topup_inputs
    else:
        return None
...
def get_eddy_topup_opt (wildcards, input):
    ...
    if num_scans > 1 and not config['no_topup']:
        topup_prefix = bids(root='work',suffix='topup',datatype='dwi',**config['subj_wildcards']).format(**wildcards)
...
def get_eddy_slspec_input (wildcards):
    if config['eddy_no_s2v']:
        ...
        return {'eddy_slspec_txt': bids(root='work',suffix='dwi.eddy_slspec.txt',desc='degibbs',datatype='dwi',**config['subj_wildcards'])}
...
rule copy_inputs_for_bedpost:
    ...
    output:
        diff_dir = directory(bids(root='work',desc='eddy',suffix='diffusion',space='T1w',res=config['resample_dwi']['resample_scheme'],datatype='dwi',**config['subj_wildcards'])),
        dwi = os.path.join(bids(root='work',desc='eddy',suffix='diffusion',space='T1w',res=config['resample_dwi']['resample_scheme'],datatype='dwi',**config['subj_wildcards']),'data.nii.gz'),
        brainmask = os.path.join(bids(root='work',desc='eddy',suffix='diffusion',space='T1w',res=config['resample_dwi']['resample_scheme'],datatype='dwi',**config['subj_wildcards']),'nodif_brain_mask.nii.gz'),
...
rule run_bedpost:
    input:
        diff_dir = bids(root='work',desc='eddy',suffix='diffusion',space='T1w',res=config['resample_dwi']['resample_scheme'],datatype='dwi',**config['subj_wildcards']),
        dwi = os.path.join(bids(root='work',desc='eddy',suffix='diffusion',space='T1w',res=config['resample_dwi']['resample_scheme'],datatype='dwi',**config['subj_wildcards']),'data.nii.gz'),
        brainmask = os.path.join(bids(root='work',desc='eddy',suffix='diffusion',space='T1w',res=config['resample_dwi']['resample_scheme'],datatype='dwi',**config['subj_wildcards']),'nodif_brain_mask.nii.gz'),
        bval = os.path.join(bids(root='work',desc='eddy',suffix='diffusion',space='T1w',res=config['resample_dwi']['resample_scheme'],datatype='dwi',**config['subj_wildcards']),'bvals'),
        bvec = os.path.join(bids(root='work',desc='eddy',suffix='diffusion',space='T1w',res=config['resample_dwi']['resample_scheme'],datatype='dwi',**config['subj_wildcards']),'bvecs'),
