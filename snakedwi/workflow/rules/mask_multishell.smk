...
checkpoint split_shell_avgs:
    input: 
        avg_4d = bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells.nii.gz',**subj_wildcards)
    params:
        out_prefix = lambda wildcards, output: os.path.join(output.nii_dir,'dwi.')
    output:
        nii_dir = directory(bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells',**subj_wildcards))
...
rule n4_shell_avg:
    input:
        avg_nii = bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells/dwi.{shell}.nii.gz',**subj_wildcards)
    output:
        n4_nii = bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells/dwi_n4.{shell}.nii.gz',**subj_wildcards)
    container: config['singularity']['prepdwi']
    shell:
...
rule n4_shell_avg_withb0mask:
    input:
        avg_nii = bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells/dwi.{shell}.nii.gz',**subj_wildcards),
        mask_nii = bids(root='work',suffix='mask.nii.gz',desc='brain',from_='avgb0',**subj_wildcards),
    output:
        n4_nii = bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells/dwi_n4withb0mask.{shell}.nii.gz',**subj_wildcards)
    container: config['singularity']['prepdwi']
    shell:
...
def get_diffweighted_shells_for_tissue_seg(wildcards):
    ...
    return sorted(expand( bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells/dwi_n4_rescale.{shell}.nii.gz',**subj_wildcards),
           **wildcards,
...
rule tissue_seg_kmeans_init:
    input: 
        ...
        mask = bids(root='work',suffix='mask.nii.gz',desc='brain',from_='avgb0',**subj_wildcards),
    params:
        intensity_images = lambda wildcards, input: ' '.join([f'--intensity-image [{img},1]' for img in input.shells]),
        k = '{k}', # 4 should be enough to get bg as the first class
        posterior_fmt = 'posteriors_%d.nii.gz',
        posterior_glob = 'posteriors_*.nii.gz',
    output:
        seg = bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells/atropos_k-{k}_initmasking_dseg.nii.gz',**subj_wildcards),
        posteriors = bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells/atropos_k-{k}_initmasking_probseg.nii.gz',**subj_wildcards)
    shadow: 'minimal'    
    container: config['singularity']['prepdwi']
    shell:
...
rule extract_posterior_bgnd:
    input:
        posterior_4d = bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells/atropos_k-{k}_initmasking_probseg.nii.gz',**subj_wildcards)
    output:
        posterior_bgnd = bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells/atropos_k-{k}_initmasking_label-bg_probseg.nii.gz',**subj_wildcards)
    container: config['singularity']['prepdwi']
    shell:
...
rule refine_mask_with_tissue_prob:
    input:  
        ...
        posterior_bgnd = bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells/atropos_k-{k}_initmasking_label-bg_probseg.nii.gz',**subj_wildcards)
    output:
        mask = bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells/atropos_k-{k}_initmasking_label-brain_probseg.nii.gz',**subj_wildcards)
    container: config['singularity']['prepdwi']
    shell:
...
rule smooth_threshold_refined_mask:
    ...
    params:
        smooth = '{smooth}'
    output:
        mask = bids(root='work',desc='topup',method='jac',suffix='dwi.avgshells/atropos_k-{k}_initmasking_label-brain_smooth-{smooth}_mask.nii.gz',**subj_wildcards)
    container: config['singularity']['prepdwi']
    shell:
