...
for i, label in enumerate(snakemake.config["tissue_labels"]):
    for k in range(tissue_k_seg.shape[3]):
        ...
        sim_prior_k[i, k] = np.sum(np.logical_and(s1, s2).flat) / np.sum(
...
for i, label in enumerate(snakemake.config["tissue_labels"]):
    ...
    # write nii to file
    print(
        "writing image at channel {} to output file: {}".format(
            label_to_k_dict[label], snakemake.output.tissue_segs[i]
        )
    )
    nib.save(
